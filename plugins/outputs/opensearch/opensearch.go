...
func (a *Opensearch) Connect() error {
	...
	if time.Duration(a.HealthCheckInterval) == 0 {
		...
		log.Printf("D! Opensearch output: disabling health check")
	}
	...
	//	return fmt.Errorf("Opensearch version not supported: %s", esVersion)
	//}

	log.Println("I! Opensearch version: " + esVersion)

	a.Client = client
	a.MajorReleaseNumber = 7
...
func (a *Opensearch) Write(metrics []telegraf.Metric) error {
	...
	if res.Errors {
		for id, err := range res.Failed() {
			log.Printf("E! Opensearch indexing failure, id: %d, error: %s, caused by: %s, %s", id, err.Error.Reason, err.Error.CausedBy["reason"], err.Error.CausedBy["type"])
			break
		}
		return fmt.Errorf("W! Opensearch failed to index %d metrics", len(res.Failed()))
	}
...
func (a *Opensearch) manageTemplate(ctx context.Context) error {
	...
	if (a.OverwriteTemplate) || (!templateExists) || (templatePattern != "") {
		...
		log.Printf("D! Opensearch template %s created or updated\n", a.TemplateName)
	} else {
		log.Println("D! Found existing Opensearch template. Skipping template management")
	}
	return nil
}
...
func (a *Opensearch) SampleConfig() string {
	return sampleConfig
}

func (a *Opensearch) Description() string {
	return "Configuration for Opensearch to send metrics to."
}
