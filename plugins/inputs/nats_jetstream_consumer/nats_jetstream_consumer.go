...
var sampleConfig = `
  ...
  servers = ["nats://localhost:4222"]
	...
	## stream to consume
  stream = "telegrafStream"

	## consumer name
	consumer = "telegraf_consumer"
...
func (n *natsConsumer) Description() string {
	return "Read metrics from NATS JetStream subject"
}
...
func (n *natsConsumer) Start(acc telegraf.Accumulator) error {
	...
	if n.conn == nil || n.conn.IsClosed() {
		...
		n.jsContext, jsContextErr = n.conn.JetStream()
		if jsContextErr != nil {
			return jsContextErr
		}
		...
		sub, err := n.jsContext.Subscribe(n.Subject, func(m *nats.Msg) {
			n.in <- m
		}, nats.Bind(n.Stream, n.Consumer))
		if err != nil {
			return err
		}
	...
	n.Log.Infof("Started the NATS JetStream consumer service, nats: %v, stream: %v, subject: %v, consumer: %v",
		n.conn.ConnectedUrl(), n.Stream, n.Subject, n.Consumer)
...
func (n *natsConsumer) clean() {
	if err := n.sub.Unsubscribe(); err != nil {
		n.Log.Errorf("Error unsubscribing from subject %s, consumer %s: %s",
			n.Subject, n.Consumer, err.Error())
	}
...
func init() {
	inputs.Add("nats_jetstream_consumer", func() telegraf.Input {
		return &natsConsumer{
			Servers:                []string{"nats://localhost:4222"},
			Secure:                 false,
			Stream:                 "telegrafStream",
			Subject:                "telegraf",
			Consumer:               "telegraf_consumers",
